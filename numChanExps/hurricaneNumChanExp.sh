...
exec > hurricaneMultiChannel.txt 2>&1
...
for m_mode in {0..3}; do
    ...
    for use_sh in false true; do
        ...
        for ((i = 0; i < ${#hurricane_file_pairs[@]}; i+=2)); do
            file_pair=(${hurricane_file_pairs[$i]})
            concatenated_fr_files+="${hurricane_path}${file_pair[0]} "
            concatenated_tf_files+="${hurricane_path}numChanTF/${file_pair[1]} "
            file_pair=(${hurricane_file_pairs[$i+1]})
            concatenated_fr_files+="${hurricane_path}${file_pair[0]} "
            concatenated_tf_files+="${hurricane_path}numChanTF/${file_pair[1]} "
                
            
            ((n=i+2))
    	    outfile="hurricane_$n"
