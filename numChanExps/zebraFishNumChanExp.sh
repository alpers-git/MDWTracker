...
exec > zebrafishMultiChannel.txt 2>&1
...
for m_mode in {0..3}; do
    ...
    for use_sh in true false; do
        ...
        for ((i = 0; i < ${#zebrafish_file_pairs[@]}; i+=2)); do
            file_pair=(${zebrafish_file_pairs[$i]})
            concatenated_fr_files+="${zebrafish_path}${file_pair[0]} "
            concatenated_tf_files+="${zebrafish_path}/numChanTF/${file_pair[1]} "
            file_pair=(${zebrafish_file_pairs[$i+1]})
            concatenated_fr_files+="${zebrafish_path}${file_pair[0]} "
            concatenated_tf_files+="${zebrafish_path}/numChanTF/${file_pair[1]} "
            
            ((n=i+2))
    	    outfile="zebrafish_$n"
