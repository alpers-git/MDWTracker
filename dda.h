...
inline __device__ void dda3(
    vec3f org, vec3f dir, float tMax, vec3ui gridSize, const Lambda &lambda, bool dbg)
{
    ...
    if (dbg)
        printf("t range for volume %f %f\n", ray_t0, ray_t1);
    if (ray_t0 > ray_t1)
        return;  // no overlap with volume

    // compute first cell that ray is in:
    vec3f org_in_volume = org + ray_t0 * dir;
    if (dbg)
        printf("org in vol %f %f %f size %i %i %i\n",
               org_in_volume.x,
               org_in_volume.y,
               org_in_volume.z,
               gridSize.x,
    ...
    if (dbg)
        printf("f_cell_end %f %f %f\n", f_cell_end.x, f_cell_end.y, f_cell_end.z);

    vec3f t_step = abs(rcp_dir);
    if (dbg)
        printf("t_step %f %f %f\n", t_step.x, t_step.y, t_step.z);
    vec3f t_next = {
        ((dir.x == 0.f) ? CUDART_INF : (abs(f_cell_end.x - org_in_volume.x) * t_step.x)),
        ((dir.y == 0.f) ? CUDART_INF : (abs(f_cell_end.y - org_in_volume.y) * t_step.y)),
        ((dir.z == 0.f) ? CUDART_INF : (abs(f_cell_end.z - org_in_volume.z) * t_step.z))};
    if (dbg)
        printf("t_next %f %f %f\n", t_next.x, t_next.y, t_next.z);
    const vec3i stop = {dir.x > 0.f ? (int)gridSize.x : -1,
                        dir.y > 0.f ? (int)gridSize.y : -1,
                        dir.z > 0.f ? (int)gridSize.z : -1};
    if (dbg)
        printf("stop %i %i %i\n", stop.x, stop.y, stop.z);
    const vec3i cell_delta = {
        (dir.x > 0.f ? +1 : -1), (dir.y > 0.f ? +1 : -1), (dir.z > 0.f ? +1 : -1)};
    if (dbg)
        printf("cell_delta %i %i %i\n", cell_delta.x, cell_delta.y, cell_delta.z);
